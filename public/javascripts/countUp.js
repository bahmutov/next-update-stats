...
function countUp(target, endVal, decimals, duration) {
    ...
    this.stepUp = function(timestamp) {
        ...
        // easing
        self.frameVal = self.easeOutExpo(progress, self.frameVal, endVal, self.duration);
        ...
        if (progress < self.duration && self.frameVal < endVal) {
    ...
    this.start = function() {
        ...
        if (!isNaN(endVal) && endVal !== null) {
            self.frameVal = +self.d.innerHTML || 0;
            requestAnimationFrame(self.stepUp);
        } else {
